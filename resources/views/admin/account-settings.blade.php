    ...
    <div class="container mt-4">
        <div class="row justify-content-center">
                            ...
                            <tbody>
                                @forelse($users as $user)
                                    <tr>
                                        ...
                                        @elseif ($user->role === 'TBI_AgriBus') bg-success
                                        @elseif ($user->role === 'TBI_TLU') bg-success
            ...
            @if (session('admin_role') === 'IPTBM' ||
                    session('admin_role') === 'TBI' ||
                    session('admin_role') === 'TBI_AGRIBUS' ||
                    session('admin_role') === 'TBI_TLU')
                            ...
                            <form action="{{ route('admin.account.update') }}" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="update_option"
                                        class="form-select @error('update_option') is-invalid @enderror" id="updateOption"
                                        required>
                                        <option value="" disabled>Select an option</option>
                                        <option value="username"
                                            {{ old('update_option') == 'username' ? 'selected' : '' }}>Username Only
                                        </option>
                                        <option value="password"
                                            {{ old('update_option') == 'password' ? 'selected' : '' }}>Password Only
                                        </option>
                                        <option value="both" {{ old('update_option') == 'both' ? 'selected' : '' }}>
                                            Username & Password</option>
                                        @if (session('admin_role') === 'KMU')
                                            <option value="role" {{ old('update_option') == 'role' ? 'selected' : '' }}>
                                                Role Only</option>
                                        @endif
                                    </select>
                                    @error('update_option')
                                ...
                                <div class="mb-3">
                                    ...
                                    <small class="text-secondary d-block">Currently logged in as:
                                        <strong>{{ session('admin_user') }}</strong></small>
                                </div>
                                ...
                                <div class="mb-3 update-username d-none">
                                    ...
                                    <input type="text" name="new_user"
                                        ...
                                        placeholder="Enter new username" value="{{ old('new_user') }}">
                                    @error('new_user')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3 update-role d-none">
                                    ...
                                    <select name="new_role" class="form-select @error('new_role') is-invalid @enderror">
                                        ...
                                        <option value="KMU" {{ old('new_role') == 'KMU' ? 'selected' : '' }}>KMU (Super
                                            Admin)</option>
                                        <option value="IPTBM" {{ old('new_role') == 'IPTBM' ? 'selected' : '' }}>IPTBM
                                        </option>
                                        <option value="TBI" {{ old('new_role') == 'TBI' ? 'selected' : '' }}>TBI
                                        </option>
                                    </select>
                                    @error('new_role')
                                        <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        @if (session('admin_role') === 'KMU')
                            <div class="mb-3">
                                ...
                                <select name="role" class="form-select @error('role') is-invalid @enderror" required>
                                    ...
                                    <option value="TBI_Agribus">TBI - AgriBus</option>
                                    <option value="TBI_TLU">TBI - TLU</option>
                    ...
                    <form action="{{ route('admin.account.update') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="update_option" id="updateOption"
                                ...
                                <option value="username" {{ old('update_option') == 'username' ? 'selected' : '' }}>
                                    Username Only</option>
                                <option value="password" {{ old('update_option') == 'password' ? 'selected' : '' }}>
                                    Password Only</option>
                                <option value="both" {{ old('update_option') == 'both' ? 'selected' : '' }}>Username &
                                    Password</option>

                                {{-- Role update visible only to KMU --}}
                                @if (session('admin_role') === 'KMU')
                                    <option value="role" {{ old('update_option') == 'role' ? 'selected' : '' }}>Role
                                        Only</option>
                                @endif
                            </select>
                            @error('update_option')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="user" id="edit_username" class="form-control"
                                @if (session('admin_role') !== 'KMU') readonly @endif>

                            <small class="text-secondary d-block mt-1">
                                Currently editing: <strong id="editing_user_label">{{ session('admin_user') }}</strong>
                            </small>
                        </div>
                        ...
                        <div class="mb-3 update-role d-none">
                            ...
                            <select name="new_role" class="form-select @error('new_role') is-invalid @enderror">
                                <option value="" disabled selected>Select role</option>
                                <option value="KMU" {{ old('new_role') == 'KMU' ? 'selected' : '' }}>KMU (Super Admin)
                                </option>
                                <option value="RESEARCH" {{ old('new_role') == 'RESEARCH' ? 'selected' : '' }}>Research
                                </option>
                                <option value="IPTBM" {{ old('new_role') == 'IPTBM' ? 'selected' : '' }}>IPTBM</option>
                                <option value="TBI" {{ old('new_role') == 'TBI' ? 'selected' : '' }}>TBI</option>
                                <option value="TBI_AGRIBUS" {{ old('new_role') == 'TBI_AGRIBUS' ? 'selected' : '' }}>TBI -
                                    AgriBus
                                </option>
                                <option value="TBI_TLU" {{ old('new_role') == 'TBI_TLU' ? 'selected' : '' }}>TBI - TLU
                                </option>

                                <option value="EXTENSION" {{ old('new_role') == 'EXTENSION' ? 'selected' : '' }}>Extension
                                </option>
                            </select>
                            @error('new_role')
                                <div class="invalid-feedback">{{ $message }}</div>
...
@push('scripts')
    ...
    <script>
        $(function() {
            // --- Toggle fields (keeps your existing behavior) ---
            function toggleFields(option) {
                const usernameFields = $('.update-username');
                const passwordFields = $('.update-password');
                const roleFields = $('.update-role');

                usernameFields.addClass('d-none');
                passwordFields.addClass('d-none');
                roleFields.addClass('d-none');

                if (option === 'username') usernameFields.removeClass('d-none');
                else if (option === 'password') passwordFields.removeClass('d-none');
                else if (option === 'both') {
                    usernameFields.removeClass('d-none');
                    passwordFields.removeClass('d-none');
                } else if (option === 'role') roleFields.removeClass('d-none');
            }

            // apply on change
            $(document).on('change', '#updateOption', function() {
                toggleFields($(this).val());
            });

            // apply on load if old value exists (server-side validation)
            const initial = $('#updateOption').val();
            if (initial) toggleFields(initial);


            // --- Fill edit modal when Edit button is clicked ---
            // Use delegated binding because DataTables may redraw rows
            $(document).on('click', '.edit-user-btn', function() {
                const $btn = $(this);
                const id = $btn.data('id');
                const user = $btn.data('user') ?? ''; // username
                const role = $btn.data('role') ?? ''; // role value
                ...
                $('#editing_user_label').text(user);

                // If new_role select exists (visible only to KMU in your blade),
                // set its value (will not throw error if element absent)
                const $newRole = $('select[name="new_role"]');
                if ($newRole.length) {
                    // make sure we set the exact option value present in DB

                    $newRole.val(role).trigger('change');
                }

                // If updateOption should default to something (e.g., none), reset it
                $('#updateOption').val('').trigger('change');
                // hide fields until an option is selected
                toggleFields('');

                // show modal (in case you rely on JS to show it)
