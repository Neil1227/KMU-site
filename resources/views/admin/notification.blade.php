        ...
        <div class="card ictv-card mt-4">
            ...
            <div class="card-header text-white d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    <a href="{{ route('admin.database.records') }}" class="btn btn-sm btn-primary" target="_blank"
                        title="View Page">
                    ...
                    <tbody>
                        @forelse ($notifications as $notification)
                            ...
                            <tr data-id="{{ $notification->id }}">
                                ...
                                <td data-full="{{ $commodity->thesis_title ?? '—' }}">
                                    {{ Str::limit($commodity->thesis_title ?? '—', 50) }}</td>
                                <td data-full="{{ $commodity->technologies ?? '—' }}">
                                    {{ Str::limit($commodity->technologies ?? '—', 50) }}</td>
                                <td data-full="{{ $commodity->technology_generator ?? '—' }}">
                                    {{ Str::limit($commodity->technology_generator ?? '—', 50) }}</td>
                                <td data-full="{{ $commodity->contact_info ?? '—' }}">
                                    {{ Str::limit($commodity->contact_info ?? '—', 50) }}</td>
                                <td data-full="{{ $commodity->type_of_technology ?? '—' }}">
                                    {{ $commodity->type_of_technology ?? '—' }}</td>
                                <td data-full="{{ $commodity->link ?? '—' }}">
                                    @if ($commodity->link)
                                        <a href="{{ $commodity->link }}" target="_blank"
                                            class="text-decoration-underline">View</a>
                                    @else
                                        —
                                    @endif
                                </td>
                                <td data-full="{{ $commodity->priority_area ?? '—' }}">
                                    {{ $commodity->priority_area ?? '—' }}</td>
                                <td class="text-center">
                                    <button class="btn btn-primary btn-sm push-to-registered" title="Register Technology"
                                        data-id="{{ $notification->id }}" data-technology="{{ $commodity->technologies }}"
