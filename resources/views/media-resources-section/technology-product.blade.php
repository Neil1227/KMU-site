            ...
            @forelse ($technologies as $index => $technology)
                <div class="col-md-4 mb-4 transition-card  {{ $index >= 6 ? 'd-none' : '' }}">
                    <div class="technology-card h-100">
                        {{-- Technology Image --}}
                        <img src="{{ $technology->poster && file_exists(storage_path('app/public/technologies/' . $technology->poster))
                            ? asset('storage/technologies/' . $technology->poster)
                            : asset('assets/img/kmlogo.png') }}"
                            class="card-img media-img" alt="{{ $technology->product ?? 'Technology Product' }}">
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="mt-auto d-flex gap-2 flex-wrap">
                                {{-- External link --}}
                                @if ($technology->link)
                                    <a href="{{ $technology->link }}" target="_blank" class="btn watch-btn">View
                                        Product</a>
                                @endif

                                {{-- Redirect to internal product page --}}
                                <a href="{{ route('technologies.show', $technology->id) }}" class="btn watch-btn">See
                                    Details</a>
                            </div>
                        </div>
